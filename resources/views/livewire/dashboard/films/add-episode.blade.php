            ...
            <div class="row">
                <div class="col-sm-12 col-md-6">
                    <div class="form-group">
                        <input name="url[]" wire:model.defer="episodes.{{ $key }}.url" rows="4" class="form-control no-resize"
                                  placeholder="Embed Code From JWPlayer Server" />
                        @error('episodes.'.$key.'.url') <span style="color: red; margin-left: 10px">{{ $message }}</span>  @enderror
                    </div>
                </div>
                <div class="col-sm-12 col-md-5">
                    <div class="form-group">
                        <input name="api_url[]" wire:model.defer="episodes.{{ $key }}.api_url" rows="4" class="form-control no-resize"
                                  placeholder="API URL" />
                        @error('episodes.'.$key.'.url') <span style="color: red; margin-left: 10px">{{ $message }}</span>  @enderror
                    </div>
                </div>
